...
#Token definition generated by Popp
...
//Lexer header generated by Popp
...
string java_regexSearch = R"(
        ...
        if (matcher.find()) {
            ...
            {1}TokDefinition tok = new {2}TokDefinition(lexeme, _Type.{3}, "");
...
map<OutputStringType, string> py = {
 ...
 map<OutputStringType, string> java = {
    ...
    {lexer_regexDefintion, "    Pattern {0}_regex = Pattern.compile(\"{1}\");\n"},
    {lexer_matchDeclaration, "\n"},
...
//Return the first set information for the non terminal which matches the name argument
...
void FileWriter::createLexer(vector<TokenRegex> tokenRegexes){
    ...
    for (auto regexDef : tokenRegexes){
        ...
        if (*language == java){
            searchCode.replace(searchCode.find("{1}"), 3, projectName);
            searchCode.replace(searchCode.find("{2}"), 3, projectName);
        }
        else{
        ...
        if (*language == java){
            searchCode.replace(searchCode.find("{3}"), 3, regexDef.name);
        }
...
void FileWriter::createParserHeader(){
    //Only needed for cpp
    parserHeaderText += "//Parser header generated by Popp\n\n";
...
void FileWriter::fileSetup(vector<FirstSetInfo> allFSInfo, set<string> allTokenTypes, vector<TokenRegex> tokenRegexes, string projName){
    ...
    //Add necessary includes for parser
    ...
    for (FirstSetInfo info : allFirstSetInfo){
        ...
        if (lookAhead == info.firstTerminals.end()){
            ...
            while (lookAhead != info.firstTerminals.end()){
                parserText += formatString((*language)[defineFirstTerminals_list], (*terminalPointer).lexeme); //Same for all langs 
                terminalPointer++;
                lookAhead++;
            }
        ...
        if (lookAhead == info.firstTokens.end()){
            ...
            while (lookAhead != info.firstTokens.end()){
                parserText += formatString((*language)[defineFirstTokens_list], (*tokenPointer).lexeme); //Same for all langs
                tokenPointer++;
                lookAhead++;
            }
    ...
    if ((*language) == cpp){ //Other languages have built in functions
...
//Creates an else condition that will handle an error when parsing
...
void FileWriter::appendCode(string code){
    ...
    //Erase the expression code and the first tokens.
